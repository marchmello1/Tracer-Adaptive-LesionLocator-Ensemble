...
cannot fragment a predicted component. Paired replay on local interactive
fixtures showed no observed FDG regression and improved one PSMA trajectory
from 0.8278 to 0.8343 AUC-Dice and from 0.9296 to 0.9502 AUC-DMM/F1. The method
...
\section{Results}
On paired local replay, the topology gate was identical to upstream on ordinary
trajectories. Safe background removal raised PSMA1 AUC-Dice from 0.8006 to
0.8011 with unchanged F1. Relaxed, topology-checked correction raised PSMA2
AUC-Dice from 0.8278 to 0.8343 and AUC-DMM/F1 from 0.9296 to 0.9502. PSMA4 was
unchanged (0.8480 Dice, 0.8611 F1), and the conservative path was retained for
the highly fragmented 178-component PSMA1 scan. These local fixtures support
selection of the submitted policy but are not a substitute for hidden-test
evaluation. The frozen container subsequently obtained preliminary-test Dice
...
on a larger independent validation set. This topology-safe derivative improved
the targeted PSMA case without observed regression in the evaluated fixtures.
